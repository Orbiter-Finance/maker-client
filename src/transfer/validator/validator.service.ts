...
import { SwapOrder, SwapOrderType, TransferAmountTransaction } from '../sequencer/sequencer.interface';
...
import { BridgeTransactionAttributes } from 'src/models';
...
export class ValidatorService {
    private logger = createLoggerByName(ValidatorService.name);
    ...
    public async transactionGetPrivateKey(transfer: TransferAmountTransaction) {
        const addressList: string[] = uniq(transfer.responseMaker || [transfer.sourceMaker]);
        const wallet = {
            address: "",
            balance: 0,
            token: null,
        ...
        const token = this.chainConfigService.getTokenByAddress(transfer.targetChain, transfer.targetToken);
        if (!token) {
            throw new Error('transactionGetPrivateKey token not found');
        }
        const transferAmount = BigInt(new BigNumber(transfer.targetAmount).times(10 ** token.decimals).toFixed(0));
        for (const address of addressList) {
            try {
                ...
                // valid balance
                const account = await this.accountFactoryService.createMakerAccount(address, transfer.targetChain);
                ...
                if (balance && balance > transferAmount) {
                    ...
                    return {
                        ...
                        balanceWei: balance,
                        balance: new BigNumber(balance.toString()).div(10 ** token.decimals).toString()
                    }
                } else {
    ...
    public async transactionGetPrivateKeys(chainId: string, token: string, transfers: TransferAmountTransaction[]) {
        const errors = [];
        const groupData = groupBy(transfers, 'responseMaker');
        const transferRelWallet: { [key: string]: TransferAmountTransaction[] } = {

        };
        const accounts: { [key: string]: OrbiterAccount } = {

        }
        const transferToken = this.chainConfigService.getTokenByAddress(chainId, token);
        if (!transferToken) {
            throw new Error(`${token} transferToken not found`)
        }
        const batchTransferCount = this.makerConfig.get(`${chainId}.BatchTransferCount`) || 1;
        const transferWalletRelAmount = {}
        for (const key in groupData) {
            const makers = key.split(',');
            const batchTransfers: TransferAmountTransaction[] = take(groupData[key], batchTransferCount);
            const totalSend: number = batchTransfers.reduce((total, current) => total + (+current.targetAmount), 0);
            const totalSendWei = new BigNumber(totalSend).times(10 ** transferToken.decimals);
            for (const address of makers) {
                ...
                const account = await this.accountFactoryService.createMakerAccount(senderAddress, chainId).connect(privateKey);
                if (account) {
                    ...
                    if (balance < totalSendWei) {
                        errors.push(`${senderAddress} Insufficient Balance ${totalSendWei}/${balance}`);
                        continue;
                    }
                    if (balance >= totalSendWei) {
                        transferWalletRelAmount[senderAddress] -= BigInt(totalSendWei.toFixed(0));
    ...
    public async validatingValueMatches(sourceSymbol:string, sourceAmount:string, targetSymbol:string,targetAmount:string) {
        const sourceAmountValue = await this.chainLinkService.getChainLinkPrice(sourceAmount, sourceSymbol, 'usd');
        const targetAmountValue = await this.chainLinkService.getChainLinkPrice(targetAmount, targetSymbol, 'usd');
        console.log(sourceAmountValue.toString(), '==sourceAmountValue', targetAmountValue.toString())
